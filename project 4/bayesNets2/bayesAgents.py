...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignmentDict in obsCPT:
                # print "hesnri", assignmentDict[GHOST_HOUSE_VAR]
                probability = 0

                #check what quadrant you are currently on
                currentQuadrant = None
                if housePos == bottomLeftPos:
                    currentQuadrant = BOTTOM_LEFT_VAL
                if housePos == topLeftPos:
                    currentQuadrant = TOP_LEFT_VAL
                if housePos == bottomRightPos:
                    currentQuadrant = BOTTOM_RIGHT_VAL
                if housePos == topRightPos:
                    currentQuadrant = TOP_RIGHT_VAL
                
                #check for all possibilities 
                if currentQuadrant != assignmentDict[GHOST_HOUSE_VAR] and currentQuadrant != assignmentDict[FOOD_HOUSE_VAR]:
                    if assignmentDict[obsVar] == RED_OBS_VAL:
                        probability = 0
                    elif assignmentDict[obsVar] == BLUE_OBS_VAL:
                        probability = 0
                    elif assignmentDict[obsVar] == NO_OBS_VAL:
                        probability = 1
                elif currentQuadrant == assignmentDict[FOOD_HOUSE_VAR] or assignmentDict[FOOD_HOUSE_VAR] == assignmentDict[GHOST_HOUSE_VAR]:
                    if assignmentDict[obsVar] == RED_OBS_VAL:
                        ...
                        probability = 0
                elif currentQuadrant == assignmentDict[GHOST_HOUSE_VAR]:
                    if assignmentDict[obsVar] == RED_OBS_VAL:
                        ...
                        probability = 0
                obsFactor.setProbability(assignmentDict, probability)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    infer = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    assignments = [var for var in infer.getAllPossibleAssignmentDicts()]
    probs = [infer.getProbability(var) for var in infer.getAllPossibleAssignmentDicts()]
    return assignments[probs.index(max(probs))]
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        infer = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        ...
        prob_left = infer.getProbability(left)
        prob_right = infer.getProbability(right)

        rightExpectedValue = (WON_GAME_REWARD * prob_right) + (GHOST_COLLISION_REWARD * (1 - prob_right))
        leftExpectedValue = (WON_GAME_REWARD * prob_left) + (GHOST_COLLISION_REWARD * (1  - prob_left))
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for probAndOutcome in probsAndOutcomes:
            expectedValues = self.computeEnterValues(probAndOutcome[1], enterEliminationOrder)
            expectedValue += max(expectedValues[0], expectedValues[1]) * probAndOutcome[0]
