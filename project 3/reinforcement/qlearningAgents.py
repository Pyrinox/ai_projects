...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
          to_max.extend([self.getQValue(state, action)])
        return max(to_max)
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in actions:
          ...
          to_max[action]= q_value
        return to_max.argMax()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for nextAction in nextActions:
          to_max.extend([self.getQValue(nextState, nextAction)])
        max_q = 0
        if len(to_max) > 0:
          max_q = max(to_max)
        ...
        for feature in features:  
          feature_val = features[feature]
          self.weights[feature] = self.weights[feature] + (self.alpha * difference * feature_val)
